...
function OnStatusUpdate(data) {
    ...
    switch (statusLabel.innerText) {
        case "READY":
            statusLabel.style.color = "rgb(0,255,0)";
            break;
        case "LOADING BOTS":
            statusLabel.style.color = "rgb(255,255,0)";
            break;
        case "UNREACHABLE":
            statusLabel.style.color = "rgb(255,0,0)";
...
function OnOverlookUpdate(data) {
    if (typeof data.accounts !== "undefined")
        document.getElementById('account-amount').innerHTML = 'Loaded accounts: <span style="color: rgb(230,252,255); padding-bottom: -155px">' + data.accounts + '</span>';
    document.getElementById('attack-amount').innerHTML = 'Ongoing attacks: <span style="color: rgb(230,252,255); padding-bottom: -155px">' + data.attacks + '</span>';
}
